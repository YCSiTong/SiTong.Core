...
namespace St.ServiceExtensions.Aop.BaseCaChe
{
    public abstract class CaCheHelper
    {
        ...
        /// <param name="invocation"></param>
        /// <returns></returns>
        protected string CaCheKey(IInvocation invocation)
        {
        ...
        protected static string GetArgumentValue(object arg)
        {
            ...
            if (arg != null)
            {
                if (arg is Expression)
                {
                    ...
                    return MD5Helper.MD5Created16(result);
                }
                else if (arg.GetType().IsClass)
                {
                    return MD5Helper.MD5Created16(Newtonsoft.Json.JsonConvert.SerializeObject(arg));
                }
            }
            return string.Empty;
        }
        ...
        private static string GetOperator(ExpressionType expressiontype)
        {
            return expressiontype switch
            {
                ...
                _ => throw new ArgumentException(string.Format("不支持{0}此种运算符查找！" + expressiontype)),
            };
        }
        ...
        private static string ResolveLinqToObject(Expression expression, object value, ExpressionType? expressiontype = null)
        {
            ...
            switch (MethodName)
            {
                ...
                case "LongCount":
                    return Len(MethodCall, value, expressiontype.Value);
                default:
                    throw new ArgumentException(string.Format("不支持{0}方法的查找！", MethodName));
            }
        }
