    ...
    public class RedisCaCheAop : CaCheHelper
    {
        ...
        public override void Intercept(IInvocation invocation)
        {
            ...
            if (method.GetCustomAttributes(true).FirstOrDefault(op => op.GetType() == typeof(StCaCheAttribute)) is StCaCheAttribute stCaChe)
            {
                ...
                var cacheValue = _RedisCaChe.GetVal(cacheKey);
                if (!string.IsNullOrEmpty(cacheValue))
                {
                    ...
                    {
                        // 核心2，要进行 ChangeType
                        response = Convert.ChangeType(_RedisCaChe.GetVal<object>(cacheKey), type);
                    }
                ...
                if (!string.IsNullOrWhiteSpace(cacheKey))
                {
                    ...
                    _RedisCaChe.SetVal(cacheKey, response, TimeSpan.FromMinutes(stCaChe.ExpirationTime));
                }
            }
            else
            {
